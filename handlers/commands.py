...
async def send_welcome(message: types.Message):
    ...
    await message.reply(
        ...
        reply_markup=kb.start_keyboard(bot_username, ref_user_id=message.from_user.id, language=lang),
        parse_mode="HTML",
    )
    ...
    if message.chat.type == ChatType.PRIVATE:
        ...
        if pending:
            ...
            await message.answer(
                bm.pending_download_status(language=lang),
                reply_markup=kb.start_private_chat_keyboard(bot_username),
            )
...
async def switch_language(call: types.CallbackQuery):
    ...
    await user_mod.db.upsert_chat(
        ...
        user_name=getattr(call.from_user, "full_name", None) or getattr(call.from_user, "username", None) or str(call.from_user.id),
        user_username=getattr(call.from_user, "username", None),
        chat_type="private",
        language=language,
        status="active",
    )
    ...
    try:
        await call.message.edit_text(
            bm.welcome_message(language=language),
            reply_markup=kb.start_keyboard(bot_username, language=language),
            parse_mode="HTML",
        )
    except Exception:
    ...
    await call.answer("Language updated" if language == "en" else "تم تحديث اللغة")
