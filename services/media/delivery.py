...
async def _send_rich_media_for_message(
    ...
    *,
    ...
    """
    Try the Rich Message path before the legacy media-group path.

    Layout:
      * 2+ photos -> Slideshow block
      * video -> Details/Toggle block
      * mixed photo/video posts -> Slideshow + Details blocks

    Guest messages cannot use send_rich_message directly. They must be
    answered exactly once through answerGuestQuery, so the same rich message
    is wrapped in an InlineQueryResultArticle.
    """
    if not entries:
        return None
    ...
    guest_id = getattr(message, "guest_query_id", None)
    if guest_id:
        from handlers.guest_rich_video import answer_guest_query_with_rich_message

        await answer_guest_query_with_rich_message(
            message.bot,
            guest_query_id=str(guest_id),
            result_id=f"guest-rich:{message.message_id}",
...
async def send_cached_media_entries(
    ...
    *,
    ...
    rich_result = await _send_rich_media_for_message(
        message,
        entries=entries,
        caption=caption,
        reply_markup=reply_markup,
